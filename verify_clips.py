...
# ---------------------------------------------------------
# FULL-SESSION SRT CROSS-CHECK (optional - blank folder skips it)
# ---------------------------------------------------------

FULL_SERMON_SRT_FOLDER = app_config.get_full_transcript_srt_folder(CONFIG)
...
# The live engine uses the faster/smaller "whisper_model" setting.
# This second pass uses "verify_model" (default "medium") for better
# accuracy re-checking the same audio.

VERIFY_MODEL = CONFIG.get("verify_model", app_config.DEFAULTS["verify_model"])

VERIFY_DEVICE = CONFIG.get("verify_device", app_config.DEFAULTS["verify_device"])

# int8_float16 saves VRAM compared with full float16 - a good choice
# while OBS may also be using the GPU.
VERIFY_COMPUTE_TYPE = CONFIG.get(
    "verify_compute_type", app_config.DEFAULTS["verify_compute_type"]
)
...
VERIFIED_SIMILARITY = int(
    CONFIG.get("verified_similarity", app_config.DEFAULTS["verified_similarity"])
)

# Average Whisper word probability in matched passage.
VERIFIED_CONFIDENCE = float(
    CONFIG.get("verified_confidence", app_config.DEFAULTS["verified_confidence"])
)
...
# =========================================================
# DOMAIN VOCABULARY (Whisper accuracy hint, per active preset)
# =========================================================
